...
export default function EditAlbumPage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // 3. Finalize and update manifest
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
          ...
          {newPhotos.map((p) => (
            <div key={p.uid} className="relative rounded-xl overflow-hidden border border-blue-500/50 bg-neutral-900 aspect-4/3">
              ...
              <div className="absolute bottom-2 left-2 px-1.5 py-0.5 rounded bg-blue-600 text-[10px] font-semibold text-white">
                New
              </div>
              <button
                type="button"
